...
import { routes, secondRoutes } from '../../pages/user/routes'
        ...
        <div className="mt-4 border-y border-stone-200 dark:border-stone-500">
          {routes.map(({ icon: Icon, ...route }) => {
            ...
            return (
              ...
              ${Router.pathname === `/user${route.path}` ? 'bg-gray-900' : 'hover:bg-gray-900'}`}>
                  <span key={route.icon} className={`text-2xl
                ${Router.pathname === `/user${route.path}` ? 'text-stone-50' : 'group-hover:text-white'}`}>
                    {Icon}
                  </span>
          ...
          {secondRoutes.map(({ icon: Icon, ...route }) => {
            return (
              ...
              hover:bg-gray-900 p-2 rounded-md group cursor-pointer hover:shadow-lg m-auto">
                ...
                ${Router.pathname === `/user${route.path}` ? 'text-white' : 'group-hover:text-white'}`}>
                    {Icon}
                  </span>
